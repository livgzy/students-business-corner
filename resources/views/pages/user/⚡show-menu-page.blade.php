...
new class extends Component
{
    ...
    public function updatedSelectedDay($day)
    {
        ...
        $slot = $this->product->tenant->pick_slot
            ->where('dayPickup', $day)
            ->first();
    ...
    public function addToCart()
    {
        ...
        $this->product->load('tenant.pick_slot');

        if (!$this->product->is_available) {
            $this->addError('quantity', 'Produk ini sudah tidak tersedia.');
            return;
        }

        if (!$this->product->tenant->is_open) {
            $this->addError('quantity', 'Tenant sedang tutup. Pemesanan tidak dapat dilakukan.');
            return;
        }

        $isPreorder = (bool) $this->product->is_preorder;
        $tenantId = (int) $this->product->tenant_id;

        if ($isPreorder) {
            if (empty($this->selectedDay)) {
                $this->addError('selectedDay', 'Mohon pilih hari pengambilan.');
                return;
            }

            if (empty($this->selectedTime)) {
                $this->addError('selectedTime', 'Mohon pilih jam pengambilan.');
                return;
            }

            $slot = $this->product->tenant->pick_slot
                ->where('dayPickup', $this->selectedDay)
                ->first();

            if (!$slot) {
                $this->addError('selectedDay', 'Jadwal pengambilan untuk hari tersebut tidak tersedia.');
                return;
            }
        ...
        foreach ($cart as $cartItem) {
            if ((int) ($cartItem['tenant_id'] ?? 0) !== $tenantId) {
                continue;
            }

            $cartItemIsPreorder = (bool) ($cartItem['is_preorder'] ?? false);

            if ($cartItemIsPreorder !== $isPreorder) {
                $existingMode = $cartItemIsPreorder ? 'Pre-order' : 'Order biasa';
                $newMode = $isPreorder ? 'Pre-order' : 'Order biasa';

                $this->dispatch('notify', 
                    message: "Menu {$newMode} tidak dapat ditambahkan. Keranjang Tenant {$this->product->tenant->tenant_code} sudah berisi {$existingMode}. Dalam satu tenant, Pre-order dan Order biasa tidak dapat digabungkan dalam satu order.", 
                    type: 'error'
                );
        ...
        if (isset($cart[$itemKey])) {
            ...
            $cart[$itemKey]['is_preorder'] = $isPreorder;
            $cart[$itemKey]['selectedDay'] = $isPreorder ? $this->selectedDay : null;
            $cart[$itemKey]['selectedTime'] = $isPreorder ? $this->selectedTime : null;
        } else {
            $cart[$itemKey] = [
                'id' => $this->product->id,
                ...
                'selectedDay' => $isPreorder ? $this->selectedDay : null,
                'selectedTime' => $isPreorder ? $this->selectedTime : null,
            ];
        }
    ...
    <div class="bg-gray-100 py-3">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav class="flex items-center space-x-2 text-sm">
                ...
                <a href="/tenant/{{ $product->tenant->tenant_code }}" class="text-gray-500 hover:text-orange-600 transition">{{ $product->tenant->store_name }}</a>
                <flux:icon.chevron-right class="size-4 text-gray-500" />
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="relative h-96 md:h-[500px] bg-gradient-to-br from-gray-100 to-gray-200">
                        @if($product->product_img)
                            <img src="{{ Storage::disk('tsbc_disk')->url($product->product_img) }}" alt="{{ $product->name }}" class="w-full h-full object-cover">
                        @else
                        ...
                        @if(!$product->is_available || !$product->tenant->is_open)
                            <div class="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center">
                                <span class="text-white text-2xl font-bold px-6 py-3 bg-red-600 rounded-full">Tidak Tersedia</span>
                            </div>
                        @endif

                        <div class="absolute top-4 right-4">
                            @if($product->is_available && $product->tenant->is_open)
                                <span class="bg-orange-500 text-white px-3 py-1 rounded-full text-sm font-semibold">Tersedia</span>
                            @else
            ...
            <div class="space-y-6">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <a href="/tenant/{{ $product->tenant->tenant_code }}" class="font-semibold text-gray-800 hover:text-orange-600 transition">{{ $product->tenant->store_name }}</a>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-2">
                        @if($product->category)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">{{ $product->category->name }}</span>
                        @endif
                    </div>
                </div>
                ...
                @if($product->is_available && $product->tenant->is_open)
                    @if($product->is_preorder)
                        <div class="bg-amber-50 border border-amber-200 rounded-xl p-5 space-y-4 shadow-sm">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-amber-800 uppercase tracking-wider mb-2">1. Pilih Hari Pengambilan:</label>
                                    ...
                                    @if($product->tenant->pick_slot->isNotEmpty())
                                        ...
                                        @foreach($product->tenant->pick_slot as $slot)
                                            ...
                                            <label class="{{ $isToday ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                                                <input type="radio" wire:model.live="selectedDay" value="{{ $slot->dayPickup }}" class="peer sr-only" @disabled($isToday)>
                                                <div class="px-4 py-2 rounded-lg text-sm font-semibold border transition-all duration-200 {{ $isToday ? 'bg-gray-100 text-gray-400 border-gray-200 opacity-60' : 'bg-white text-amber-900 border-amber-200 shadow-sm hover:bg-amber-100' }} peer-checked:bg-amber-600 peer-checked:text-white peer-checked:border-amber-600 peer-checked:shadow-md">
                                                    {{ $slot->dayPickup }}
                                                </div>
                                            </label>
                                        @endforeach
                            ...
                            @if(!empty($selectedDay))
                                <div class="pt-2 border-t border-amber-200/60 animate-fade-in">
                                    <div class="flex justify-between items-center mb-1.5">
                                        <label for="pickup_time" class="block text-xs font-semibold text-amber-800 uppercase tracking-wider">2. Pilih Jam Pengambilan (Hari {{ $selectedDay }}):</label>
                                        <span wire:loading wire:target="selectedDay" class="text-xs text-amber-600 italic">Memuat jam...</span>
                                    </div>

                                    <div wire:loading.remove wire:target="selectedDay">
                                        <x-time-input model="selectedTime" min="{{ $slotStart }}" max="{{ $slotEnd }}" />
                                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex items-center border border-gray-300 rounded-lg">
                                <button wire:click="decrementQuantity" wire:loading.attr="disabled" class="px-4 py-2 text-gray-600 hover:bg-gray-100 transition rounded-l-lg">
                                    <flux:icon.minus />
                                </button>
                                <span class="px-6 py-2 text-center font-semibold min-w-[60px]">{{ $quantity }}</span>
                                <button wire:click="incrementQuantity" wire:loading.attr="disabled" class="px-4 py-2 text-gray-600 hover:bg-gray-100 transition rounded-r-lg">
                                    <flux:icon.plus />
                                </button>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea wire:model="specialInstructions" rows="3" placeholder="Contoh: Tidak pedas, tambah sambal, dll..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"></textarea>
                    </div>
                    ...
                    <div class="space-y-3">
                        @auth
                            @if($product->is_preorder)
                                @if($product->tenant->pick_slot->isNotEmpty())
                                    <button wire:click="addToCart" wire:loading.attr="disabled" class="w-full bg-amber-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-orange-700 transition flex items-center justify-center space-x-2 cursor-pointer">
                                        <flux:icon.shopping-cart />
                                        <span>Tambah ke Keranjang</span>
                                    </button>
                                @else
                                    <button disabled class="w-full border-2 border-orange-600 text-orange-600 px-6 py-3 rounded-lg font-semibold bg-gray-100 cursor-not-allowed opacity-75">
                                        Menu Belum Bisa dipesan
                                    </button>
                                @endif
                            @else
                                <button wire:click="addToCart" wire:loading.attr="disabled" class="w-full bg-orange-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-orange-700 transition flex items-center justify-center space-x-2 cursor-pointer">
                                    <flux:icon.shopping-cart />
                                    <span>Tambah ke Keranjang</span>
                                </button>
                            @endif
        ...
        @if(count($this->recomendedMenu) > 0)
            <div class="mt-16">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a wire:navigate href="/menu" class="text-orange-600 hover:text-orange-700 font-medium transition">Lihat Semua Menu →</a>
                </div>
                            ...
                            <div>
                                <a wire:navigate href="/menu/{{ $recommended->slug }}" class="block relative h-48 bg-gray-200 overflow-hidden">
                                    @if($recommended->product_img)
                                        <img src="{{ Storage::disk('tsbc_disk')->url($recommended->product_img) }}" alt="{{ $recommended->name }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
                                    @else
                                ...
                                <div class="p-4 pb-0">
                                    <a wire:navigate href="/menu/{{ $recommended->slug }}" class="block">
                                        <h3 class="font-semibold text-gray-800 group-hover:text-orange-600 transition line-clamp-1">{{ $recommended->name }}</h3>
                                        <p class="text-sm text-gray-500 mt-1">{{ $recommended->tenant->store_name }}</p>
                                    </a>

                                    <div class="flex items-center justify-between mt-2">
                                        <span class="font-bold text-gray-900">{{ $recommended->formatted_price }}</span>
                                    </div>
                                </div>
                            </div>
                                                ...
                                                <div class="flex flex-wrap gap-1">
                                                    @if($recommended->tenant->pick_slot->isNotEmpty())
                                                        @foreach($recommended->tenant->pick_slot->take(2) as $day)
                                                            <span class="bg-orange-100 text-orange-700 px-1.5 py-1 rounded text-[12px] mt-1 font-medium">{{ $day->dayPickup }}</span>
                                                        @endforeach

                                                        @if($recommended->tenant->pick_slot->count() >= 3)
                                                            <div class="text-gray-500 text-[12px] font-bold mt-1 select-none clear-both">...</div>
                                                        @endif
                                                    @else
